...
async function main() {
  ...
  console.log(`📁 Kategorie "auto-motorrad": ${autoMotorradCategory ? `✅ Gefunden (ID: ${autoMotorradCategory.id}, slug: ${autoMotorradCategory.slug})` : '❌ NICHT GEFUNDEN'}\n`)
  ...
  motorcycles.forEach((motorcycle, index) => {
    ...
    if (motorcycle.categories.length === 0) {
      ...
      motorcycle.categories.forEach((wc: any) => {
        ...
        const isAutoMotorrad = cat.slug === 'auto-motorrad' || cat.name.toLowerCase().includes('motorrad')
  ...
  if (autoMotorradCategory) {
    ...
    const motorcyclesWithoutCategory = motorcycles.filter(m => 
      !m.categories.some((wc: any) => wc.category.slug === 'auto-motorrad')
    )
