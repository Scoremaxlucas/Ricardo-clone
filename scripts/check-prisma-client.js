...
if (prisma.contactRequest) {
  ...
  console.log('Verfügbare Methoden:', Object.keys(prisma.contactRequest).slice(0, 5).join(', '), '...')
  
  // Teste eine Query
  prisma.contactRequest.findMany({ take: 1 })
    .then(result => {
  ...
  console.log('Verfügbare Modelle:', Object.keys(prisma).filter(key => !key.startsWith('$')).join(', '))
