...
export function validateQRBill(qrString: string): QRBillValidationResult {
  ...
  if (!fields.iban || fields.iban.length === 0) {
    ...
    if (!ibanRegex.test(fields.iban)) {
      warnings.push(`IBAN-Format könnte ungültig sein: ${fields.iban} (erwartet: CH + 2 Ziffern + 17 alphanumerische Zeichen)`)
    }
  }
  ...
  if (fields.creditorAddressType !== 'K') {
    errors.push(`Ungültiger Creditor Address Type: ${fields.creditorAddressType} (erwartet: K für strukturiert)`)
  }
  ...
  if (fields.debtorAddressType && fields.debtorAddressType !== 'K' && fields.debtorAddressType !== '') {
    errors.push(`Ungültiger Debtor Address Type: ${fields.debtorAddressType} (erwartet: K oder leer)`)
  }
  ...
  if (fields.debtorAddressType === 'K') {
    ...
    if (fields.debtorCountry && fields.debtorCountry !== 'CH' && fields.debtorCountry.length === 2) {
      warnings.push(`Debtor Country ist nicht CH: ${fields.debtorCountry} (für Swiss QR-Bill sollte es CH sein)`)
    }
  ...
  if (!fields.reference || fields.reference.length === 0) {
    ...
    if (!/^[0-9A-Za-z]+$/.test(referenceWithoutSpaces)) {
      errors.push(`Reference enthält ungültige Zeichen: ${fields.reference} (nur alphanumerisch erlaubt)`)
    }
  }
