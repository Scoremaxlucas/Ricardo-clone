...
export async function generateInvoicePaymentInfo(invoiceId: string): Promise<InvoicePaymentInfo> {
  ...
  const debtorName = hasDebtorInfo 
    ? (invoice.seller.name || `${invoice.seller.firstName || ''} ${invoice.seller.lastName || ''}`.trim() || invoice.seller.companyName || '')
    : ''
...
function generateQRCodeString(params: {
  ...
  if (hasDebtorInfo) {
    ...
    if (/^[A-Z]{2}$/.test(debtorCountryRaw)) {
      ...
      if (debtorCountryRaw !== 'CH') {
        console.warn('[invoice-payment-info] ⚠️  Debtor Country korrigiert von', debtorCountryRaw, 'zu CH (für Swiss QR-Bill)')
      }
    } else {
      finalDebtorCountry = 'CH'
    }
  ...
  if (cleanIban.length > 21) {
    ...
    console.error('[invoice-payment-info] ⚠️  IBAN ist zu kurz:', cleanIban.length, 'Zeichen, erwartet: 21')
  ...
  if (cleanIban.length !== 21) {
    console.error('[invoice-payment-info] ❌ IBAN hat ungültige Länge:', cleanIban.length, 'erwartet: genau 21')
  }
  ...
  if (qrLines.length !== 24) {
    console.error('[invoice-payment-info] ⚠️  QR-String hat falsche Anzahl Zeilen:', qrLines.length, 'erwartet: 24')
    console.error('[invoice-payment-info] Zeilen:', qrLines.map((line, i) => `${i + 1}: ${line}`).join('\n'))
  }
