...
export default async function MatchingMatchObjekteListPage() {
  const session = await getServerSession(authOptions)
  const userId = (session?.user as { id?: string } | undefined)?.id
  if (!userId) {
    redirect('/login?callbackUrl=' + encodeURIComponent('/matching/match-objekte'))
  }
      ...
      {properties.length === 0 ? (
        <div className="mt-12 rounded-xl border border-dashed border-slate-200 bg-slate-50/80 px-6 py-12 text-center">
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-3">
            <Link href="/matching/match-objekte/new" className="text-sm font-semibold text-teal-800 underline">
              Objekt erfassen →
            </Link>
            <Link href="/matching/match-objekte/import" className="text-sm font-semibold text-teal-800 underline">
              Zum Import →
            </Link>
          </div>
        </div>
          ...
          {properties.map(p => (
            <li key={p.id} className="flex flex-col gap-3 px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
              <div className="min-w-0">
                <Link
                  href={`/matching/match-objekte/${p.id}/edit`}
