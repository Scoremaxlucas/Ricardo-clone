...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `[check-contact-deadline] Gefunden: ${purchasesNeedingWarning.length} Purchases benötigen Warnung`
    )
    ...
    for (const purchase of purchasesNeedingWarning) {
      try {
        const daysUntilDeadline = Math.ceil(
          (purchase.contactDeadline.getTime() - now.getTime()) / (24 * 60 * 60 * 1000)
        )
        ...
        if (!purchase.sellerContactedAt) {
          try {
            ...
            try {
              ...
              console.error(
                '[check-contact-deadline] Fehler beim Senden der Warnungs-E-Mail an Verkäufer:',
                emailError
              )
            }
          } catch (error) {
            console.error(
              '[check-contact-deadline] Fehler beim Erstellen der Warnung für Verkäufer:',
              error
            )
          }
        }
        ...
        if (!purchase.buyerContactedAt) {
          try {
            ...
            try {
              ...
              console.error(
                '[check-contact-deadline] Fehler beim Senden der Warnungs-E-Mail an Käufer:',
                emailError
              )
            }
          } catch (error) {
            console.error(
              '[check-contact-deadline] Fehler beim Erstellen der Warnung für Käufer:',
              error
            )
          }
        }
    ...
    console.log(
      `[check-contact-deadline] Gefunden: ${purchasesMissedDeadline.length} Purchases mit überschrittener Frist`
    )
