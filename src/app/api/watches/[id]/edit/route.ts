...
/**
 * RICARDO-STYLE: Bearbeitung von Artikeln
 * 
 * Regeln:
...
export async function PUT(
  ...
  try {
    ...
    // RICARDO-STYLE: Prüfe ob bereits ein aktiver Kauf stattgefunden hat
    ...
    // RICARDO-STYLE: Bei vorhandenen Geboten nur erlaubte Felder
    if (hasBids) {
      ...
      if (attemptedChanges.length > 0) {
        return NextResponse.json(
          { 
            message: 'Bei vorhandenen Geboten können nur Beschreibung, Bilder und Video ergänzt werden. Preis, Auktionsdauer und andere Felder sind gesperrt.',
    ...
    if (hasBids) {
      ...
      // Auktion
      if (data.isAuction !== undefined) updateData.isAuction = data.isAuction === true || data.isAuction === 'true'
      ...
      if (data.autoRenew !== undefined) updateData.autoRenew = data.autoRenew === true || data.autoRenew === 'true'
      ...
      // Lieferumfang
      if (data.fullset !== undefined) updateData.fullset = data.fullset === true || data.fullset === 'true'
      if (data.onlyBox !== undefined) updateData.box = (data.onlyBox === true || data.onlyBox === 'true')
      if (data.onlyPapers !== undefined) updateData.papers = (data.onlyPapers === true || data.onlyPapers === 'true')
      ...
      if (data.hasWarranty !== undefined) {
        updateData.warranty = (data.hasWarranty === true || data.hasWarranty === 'true') ? 'Herstellergarantie' : null
      }
      ...
      // Versand
      if (data.shippingMethods !== undefined) {
        updateData.shippingMethod = Array.isArray(data.shippingMethods) && data.shippingMethods.length > 0
