...
export async function GET(request: NextRequest) {
  try {
    ...
    const [
      ...
      prisma.purchase.count({ where: { status: { in: ['pending', 'payment_confirmed', 'shipped'] } } }),
    ...
    const averagePurchasePrice = completedPurchases > 0 
      ? completedRevenue / completedPurchases 
      : 0

    // Berechne Erfolgsrate
    const successRate = totalWatches > 0 
      ? (soldWatches / totalWatches) * 100 
      : 0
    ...
    const averageSaleDuration = saleDurations.length > 0
