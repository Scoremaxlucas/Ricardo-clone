...
import { validateQRBill, formatValidationResult } from '@/lib/qr-bill-validator'
...
export async function GET(
  ...
  try {
    ...
    // Prüfe ob es eine Credit Note ist
    const isCreditNote = invoice.invoiceNumber.startsWith('KORR-') || invoice.total < 0 || (invoice.status === 'cancelled' && invoice.refundedAt)
    ...
    // ============================================
    // HEADER - Professioneller Header wie Ricardo
    // ============================================
    ...
    pdf.setFontSize(16)
    pdf.setFont('helvetica', 'bold')
    pdf.text(isCreditNote ? 'Korrektur-Abrechnung' : 'Rechnung', pageWidth - margin, 25, { align: 'right' })
    ...
    yPos += 5
    pdf.text(`${PAYMENT_CONFIG.address.street} ${PAYMENT_CONFIG.address.streetNumber}`, margin, yPos)
    yPos += 5
    ...
    pdf.setFont('helvetica', 'normal')
    pdf.setTextColor(100, 100, 100)
    pdf.text(`Datum: ${new Date(invoice.createdAt).toLocaleDateString('de-CH')}`, pageWidth - margin, yPos, { align: 'right' })
    yPos += 5
    if (!isCreditNote) {
      pdf.text(`Fälligkeitsdatum: ${new Date(invoice.dueDate).toLocaleDateString('de-CH')}`, pageWidth - margin, yPos, { align: 'right' })
    }
    ...
    pdf.text(`MwSt (${(invoice.vatRate * 100).toFixed(1)}%):`, pageWidth - margin - 50, yPos, { align: 'right' })
    pdf.text(`CHF ${invoice.vatAmount.toFixed(2)}`, pageWidth - margin - 2, yPos, { align: 'right' })
    yPos += 8
    ...
    pdf.text(`CHF ${Math.abs(invoice.total).toFixed(2)}`, pageWidth - margin - 2, yPos, { align: 'right' })
    ...
    if (isCreditNote) {
      ...
      pdf.text('(Gutschrift - wird automatisch verrechnet)', pageWidth - margin - 2, yPos, { align: 'right' })
    }
    ...
    if (!isCreditNote) {
      ...
      // Hintergrund für Zahlungsbereich (wie Ricardo)
      ...
      try {
        ...
        const hasDebtorInfo = !!(invoice.seller.firstName || invoice.seller.lastName || invoice.seller.companyName || invoice.seller.name)
        ...
        const debtorStreet = hasDebtorInfo && invoice.seller.street?.trim() 
          ? `${invoice.seller.street.trim()} ${invoice.seller.streetNumber?.trim() || ''}`.trim()
          : ''
        const debtorPostalCode = hasDebtorInfo && invoice.seller.postalCode?.trim() 
          ? invoice.seller.postalCode.trim() 
          : ''
        const debtorCity = hasDebtorInfo && invoice.seller.city?.trim() 
          ? invoice.seller.city.trim() 
          : ''
        const debtorCityLine = debtorPostalCode && debtorCity 
          ? `${debtorPostalCode} ${debtorCity}`.trim()
          : ''
        ...
        if (hasDebtorInfo) {
          ...
          if (/^[A-Z]{2}$/.test(debtorCountryClean)) {
            ...
            if (debtorCountryClean !== 'CH') {
              console.warn('[QR-Bill] ⚠️  Debtor Country korrigiert von', debtorCountryClean, 'zu CH (für Swiss QR-Bill)')
            }
          } else {
            debtorCountry = 'CH'
          }
        ...
        if (formattedReference.length !== 25) {
          console.error('[QR-Bill] ⚠️  Referenz hat ungültige Länge:', formattedReference.length, 'erwartet: 25')
        }
        ...
        if (cleanIban.length > 21) {
          ...
          console.error('[QR-Bill] ⚠️  IBAN ist zu kurz:', cleanIban.length, 'Zeichen, erwartet: 21')
        ...
        if (cleanIban.length !== 21) {
          console.error('[QR-Bill] ❌ IBAN hat ungültige Länge:', cleanIban.length, 'erwartet: genau 21')
        }
        ...
        if (qrLines.length !== 24) {
          console.error('[QR-Bill] ⚠️  QR-String hat falsche Anzahl Zeilen:', qrLines.length, 'erwartet: 24')
          console.error('[QR-Bill] Zeilen:', qrLines.map((line, i) => `${i + 1}: ${line}`).join('\n'))
        }
      ...
      pdf.setFont('helvetica', 'normal')
      pdf.setTextColor(100, 100, 100)
      pdf.text('Bitte überweisen Sie den Betrag bis zum Fälligkeitsdatum.', margin + 5, paymentStartY + 75)
      pdf.text('Verwenden Sie die Referenz bei der Überweisung, damit die Zahlung zugeordnet werden kann.', margin + 5, paymentStartY + 80)
    } else {
      // Bei Credit Notes: Hinweis statt Zahlungsinformationen
      yPos += 20
      ...
      pdf.text('Korrektur-Abrechnung / Gutschrift', margin + 5, yPos + 10)
      yPos += 15
      pdf.setFontSize(9)
      pdf.setFont('helvetica', 'normal')
      pdf.setTextColor(0, 0, 0)
      pdf.text('Diese Korrektur-Abrechnung stellt eine Gutschrift dar.', margin + 5, yPos)
      yPos += 6
      pdf.text('Es ist keine Zahlung erforderlich. Der Betrag wird Ihnen', margin + 5, yPos)
      yPos += 6
      pdf.text('automatisch gutgeschrieben oder mit einer offenen Rechnung verrechnet.', margin + 5, yPos)
    }
